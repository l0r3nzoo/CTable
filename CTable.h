...
int add_column(Table* self,char* column_name,DataType data_type,ConstraintType constrain_type,int nullable,void* default_value);
...
int add_column(Table* self,char* column_name,DataType data_type,ConstraintType constrain_type,int nullable,void* default_value){
    if(self==NULL){
        puts("table cannot be null");
        return 1;
    }
    if(self->NoOfColumms==self->ColumnsCap){
        size_t temp_cap=self->ColumnsCap+INITIAL_COL_CAP;
        Column* temp_allocation=(Column*)realloc(self->Columns,sizeof(Column)*temp_cap);
